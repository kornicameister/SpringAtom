...
define(
    ...
    function textInfoPageAttribute() {
        var getTemplate = function getTemplate() {
            ...
            array.push('<dynamic-grid config="conf"></dynamic-grid>');
            array.push('</div>');
            return array.join('');
        };
        return {
            name      : 'gridInfopageAttribute',
            definition: function infopageAttributeDirective() {
                return {
                    ...
                    controller: function ($scope, componentResource) {
                        var attr = $scope.attribute;
                        $scope.label = attr.label;
                        $scope.conf = componentResource.getTableComponentConfiguration(attr.value.builderId, attr.value.oid).$object
                    },
