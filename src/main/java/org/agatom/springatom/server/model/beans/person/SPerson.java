...
public class SPerson
    ...
    private List<SPersonContact> contacts;
    ...
    public boolean removeContact(final Collection<? extends SContact> contacts) {
        if (contacts.size() > 0 && this.contacts != null) {
            final Set<? extends SContact> thisContacts = Sets.newHashSet(this.getContacts());
            final Set<? extends SContact> contactSet = Sets.newHashSet(contacts);
            final Set<? extends SContact> difference = Sets.union(thisContacts, contactSet).immutableCopy();
