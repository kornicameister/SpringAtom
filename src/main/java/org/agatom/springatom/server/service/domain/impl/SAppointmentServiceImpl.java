...
public class SAppointmentServiceImpl
    ...
    @Override
    public SEnumeration getAppointmentTypes() {
        // TODO add reading from database for key = "AppointmentTaskType"
        final SEnumeration enumeration = new SEnumeration();
        final AppointmentTaskType[] taskTypes = AppointmentTaskType.values();
        for (AppointmentTaskType taskType : taskTypes) {
            enumeration.addEntry(new SEnumerationEntry().setKey(taskType.name()));
        }
        return enumeration;
    }
