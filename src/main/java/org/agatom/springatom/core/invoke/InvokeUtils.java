...
public class InvokeUtils {
	...
	private static final String IS_GETTER     = "is%s";
	...
	public static Object invokeGetter(Object target, final String property) {
		...
		for (final String prop : props) {
			Method method = ReflectionUtils.findMethod(target.getClass(), InvokeUtils.getGetterName(prop));
			if (method != null) {
				ReflectionUtils.makeAccessible(method);
				value = ReflectionUtils.invokeMethod(method, target);
			} else {
				method = ReflectionUtils.findMethod(target.getClass(), InvokeUtils.getBooleanGetterName(prop));
				if (method != null) {
					ReflectionUtils.makeAccessible(method);
					value = ReflectionUtils.invokeMethod(method, target);
				}
